...
# Must match the model config's max_token_len (pytorch_to_onnx.py bakes this length into the
# ONNX graph's attention-mask constants during export). A mismatch here makes TensorRT reject
# the graph with "IElementWiseLayer /And_*: broadcast dimensions must be conformable", since the
# profile's lang_tokens/lang_masks length no longer agrees with the exported mask length.
# pi05_libero's max_token_len is 200; update this if you export a config with a different value.
